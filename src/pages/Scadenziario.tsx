...
const Scadenziario = () => {
  ...
  // Raggruppa incassate per trimestre
  const incassatePerTrimestre = fattureIncassate.reduce((acc, f) => {
    ...
    <MainLayout>
      <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold">Scadenziario Clienti</h1>
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm text-muted-foreground">Provv. Potenziale</p>
                  <p className="text-2xl font-bold">€{provvigionePotenziale.toLocaleString('it-IT', { minimumFractionDigits: 2 })}</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="pt-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold">€{totaleIncassato.toLocaleString('it-IT', { minimumFractionDigits: 2 })}</p>
                </div>
              </div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="pt-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm text-muted-foreground">Provv. Maturata</p>
                  <p className="text-2xl font-bold text-green-600">€{provvigioneMaturata.toLocaleString('it-IT', { minimumFractionDigits: 2 })}</p>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        <Tabs defaultValue="scadenziario" className="space-y-4">
          <TabsList>
            <TabsTrigger value="scadenziario" className="gap-2">
              ...
              Scadenziario ({fattureScadute.length})
            </TabsTrigger>
            <TabsTrigger value="incassate" className="gap-2">
              <CheckCircle2 className="h-4 w-4" />
              Provvigioni Incassate ({fattureIncassate.length})
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="incassate">
            <div className="space-y-6">
              {loadingIncassate ? (
                <p className="text-muted-foreground py-8 text-center">Caricamento...</p>
              ) : fattureIncassate.length === 0 ? (
                <Card>
                  <CardContent className="py-12 text-center">
                    <TrendingUp className="h-12 w-12 mx-auto text-muted-foreground mb-4" />
                    <p className="text-muted-foreground">Nessuna fattura incassata</p>
                  </CardContent>
                </Card>
              ) : (
                Object.entries(incassatePerTrimestre)
